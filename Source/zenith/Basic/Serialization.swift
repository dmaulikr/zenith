...
extension FileHandle {
    ...
    func write<T: Serializable>(_ array: [T]) {
        write(array.count)
    ...
    func read<T: Serializable>(_ array: inout [T], elementInitializer: () -> T) {
        var count = 0
        read(&count)
        array = [T]()
    ...
    func write<T: Serializable>(_ serializable: T) {
    ...
    func write(polymorphicSerializable: Serializable) {
        write(String(describing: type(of: polymorphicSerializable)))
        polymorphicSerializable.serialize(to: self)
    }

    func read<T: Serializable>(_ serializable: inout T) {
        serializable.deserialize(from: self)
    }

    func read<T: Serializable>(elementInitializer: () -> T) -> [T] {
        var count = 0
        read(&count)
        var array = [T]()
        array.reserveCapacity(count)
    ...
    func read<T: Serializable>(_ optional: inout T?, elementInitializer: () -> T) {
    ...
    func write<Key: Serializable, Value: Serializable>(_ dictionary: [Key: Value]) {
    ...
    func read<Key, Value>(_ dictionary: inout [Key: Value], keyInitializer: () -> Key, valueInitializer: () -> Value)
